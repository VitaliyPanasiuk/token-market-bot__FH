...
async def get_change_text(message, state=changetext.get_description):
    ...
    await postgre_db.sql_change_texts(data['name'], data['get_description'])
    await state.finish()
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        on_startup_button, lambda c: c.data == 'on_startup')
    dp.register_callback_query_handler(
        instruction_button, lambda c: c.data == 'instruction')
    dp.register_callback_query_handler(
        contacts_button, lambda c: c.data == 'contacts')
    dp.register_callback_query_handler(
        price1_button, lambda c: c.data == 'price1')
    dp.register_callback_query_handler(
        price2_button, lambda c: c.data == 'price2')
    dp.register_callback_query_handler(
        price3_button, lambda c: c.data == 'price3')
    dp.register_message_handler(
        get_change_text, state=changetext.get_description)
