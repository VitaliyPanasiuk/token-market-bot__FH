...
async def start_command(message: types.Message):
    ...
    if chek_auf_user(userid) == False:
        ...
        await postgre_db.sql_add_user(data)
...
async def service1_message(message: types.Message):
    ...
    # проверяем баланс если все хорошо то списывваем денбги с баланса и выдаем токен
    if int(balance) >= int(get_text('price_first')):
        ...
        await postgre_db.sql_add_transaction(data)  # внесение в базу данных
    else:
...
async def get_chek_message(message: types.Message, state=donateState.get_chek):
    ...
    if message.text != 'Отмена':
        ...
        # внесение пополненния в базу данных
        await postgre_db.sql_add_donate(data)
        await state.finish()
    else:
        await bot.send_message(userid, 'вы отменили транзакцию', reply_markup=start_markup())
        await state.finish()
